...
# Subset the dataset the median of excellent responses in 2002 
brfss_tidy %>% 
  group_by(year) %>% 
  summarize(median_ex = median(excellent, na.rm = T)) %>% 
...
* I used filter() to make a subset of BRFSS data which only contains data in 2002. The median of the 'excellent' respsonse value is `r median(brfss_12$excellent, na.rm = T)`. Note that I removed two missing values.  
